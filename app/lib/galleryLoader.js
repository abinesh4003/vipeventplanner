// Utility to load all images and videos from public folder
export const loadGalleryAssets = () => {
  return {
    decorations: {
      images: [
        '/services/dec/stage1.jpeg',
        '/services/dec/stage2.jpeg',
        '/services/dec/stage3.jpeg',
        '/services/dec/stage4.jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.28.33 AM.jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.28.34 AM.jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.28.35 AM (1).jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.28.35 AM.jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.29.36 AM.jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.54.04 AM (1).jpeg',
        '/services/dec/WhatsApp Image 2025-11-09 at 11.54.04 AM.jpeg',
        '/services/dec/birthday/birthday.webp',
        '/services/dec/car/car1.jpg',
        '/services/dec/car/car2.jpg',
        '/services/dec/car/cardec.webp',
        '/services/dec/entrance/entrance.webp',
        '/services/dec/entrance/entrance1.jpg',
        '/services/dec/home/home.jpg',
        '/services/dec/mandap/mandp.webp',
        'services/dec/theme/theme.jpg',
      ],
      videos: [
        '/services/dec/decvid1.mp4',
        '/services/dec/decvid2.mp4',
        '/services/dec/decvid3.mp4',
        '/services/dec/decvid4.mp4',
        '/services/dec/birthday/birthday.mp4'
      ]
    },
    catering: {
      images: [
        '/services/cat/cat1.jpg',
        '/services/cat/cat2.jpeg',
        '/services/cat/cat3.jpeg',
        '/services/cat/cat4.jpeg',
        '/services/cat/biriyani/biriyani.webp',
        '/services/cat/food/diet.jpg',
        '/services/cat/food/meal.jpg',
        '/services/cat/staff/staff.webp',
        '/services/cat/nonveg.jpg',
        '/services/buffet.jpg'
      ],
      videos: [
        '/services/cat/catvid.mp4',
        '/services/cat/catvid1.mp4',
        '/services/cat/catvid2.mp4',
        '/services/cat/catvid3.mp4',
        '/services/cat/catvid4.mp4'
      ]
    },
    stalls: {
      images: [
        '/services/stalls/desserts.webp',
        '/services/stalls/drinks.webp',
        '/services/stalls/icecream.jpg',
        '/services/stalls/livefood.jpg',
        '/services/stalls/popcorn.jpg',
        '/services/stalls/special.webp',
        '/images/popcorn-stall.jpg',
        '/images/cotton-candy-stall.jpg',
        '/images/chocolate-fountain.jpg',
        '/images/ice-cream-stall.jpg',
        '/images/pani-puri-stall.jpg',
        '/images/wine-counter.jpg',
        '/images/mocktails.jpg',
        '/images/printed-balloons.jpg'
      ],
      videos: []
    },
    welcomeDolls: {
      images: [
        '/services/doll/doll1.jpg',
        '/services/doll/doll2.jpeg',
        '/services/doll/doll3.jpeg'
      ],
      videos: []
    },
    entertainment: {
      images: [
        '/services/dj/dj1.jpg',
        '/services/dj/dj2.jpeg',
        '/services/photo.jpg',
        '/services/games.jpg'
      ],
      videos: [
        '/services/dj/djvid.mp4',
        '/services/dj/djvid1.mp4'
      ]
    }
  };
};
