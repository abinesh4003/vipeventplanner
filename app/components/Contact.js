        ...
        {toast && (
          ...
          >
            <div className={`rounded-xl shadow-2xl p-4 flex items-start gap-3 ${toast.type === 'success'
              ...
              <p className={`flex-1 font-medium ${toast.type === 'success' ? 'text-[var(--color-dark)]' : 'text-red-800'
                }`}>
                {toast.message}
              </p>
              <button
                onClick={() => setToast(null)}
                className={`flex-shrink-0 ${toast.type === 'success' ? 'text-[var(--color-dark)]' : 'text-red-600'
                  } hover:opacity-70`}
              >
                <X className="w-5 h-5" />
              </button>
